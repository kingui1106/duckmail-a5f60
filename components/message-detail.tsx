...
export default function MessageDetail({ message, onBack, onDelete }: MessageDetailProps) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 md:p-6 bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-100">
      <div className="flex justify-between items-center mb-4">
        <Button variant="light" startContent={<ArrowLeft size={18} />} onPress={onBack}>
          {currentLocale === "en" ? "Back" : "返回"}
        </Button>
        <div className="flex gap-2">
          <Button variant="light" color="danger" startContent={<Trash2 size={18} />} onPress={handleDelete}>
            {currentLocale === "en" ? "Delete" : "删除"}
          </Button>
          {messageDetail.downloadUrl && (
            ...
            >
              {currentLocale === "en" ? "Download" : "下载"} (.eml)
            </Button>
          )}
        </div>
      </div>

      <Card className="mb-4 shadow-lg border border-gray-200 dark:border-gray-700">
        <CardBody className="p-6">
          <div className="mb-6 pb-4 border-b border-gray-200 dark:border-gray-700">
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-3">{messageDetail.subject}</h1>
            <div className="flex justify-between items-center">
              <div className="flex items-center">
                <Avatar name={fromInitials} size="md" className="mr-3" />
                <div>
                  <p className="font-semibold text-gray-800 dark:text-gray-200">{fromName}</p>
                  <p className="text-sm text-gray-500 dark:text-gray-400">{messageDetail.from.address}</p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-[auto,1fr] gap-x-2">
              ...
              {messageDetail.cc && messageDetail.cc.length > 0 && (
                <>
                  ...
                  <span className="text-gray-700 dark:text-gray-300">{messageDetail.cc.join(", ")}</span>
                </>
              )}
              {messageDetail.bcc && messageDetail.bcc.length > 0 && (
                <>
                  ...
                  <span className="text-gray-700 dark:text-gray-300">{messageDetail.bcc.join(", ")}</span>
                </>
              )}
            </div>
          </div>

          <div className="prose prose-sm sm:prose dark:prose-invert max-w-none mt-6 border-t border-gray-200 dark:border-gray-700 pt-6">
            {messageDetail.html && messageDetail.html.length > 0 ? (
              <div dangerouslySetInnerHTML={{ __html: messageDetail.html.join("") }} />
            ) : (
              <pre className="whitespace-pre-wrap font-sans">{messageDetail.text}</pre>
            )}
          </div>
